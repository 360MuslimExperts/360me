        ...
        >
            ...
            <div className="relative w-32 h-32 mb-4 z-10">
                ...
                <div className="relative w-full h-full rounded-full overflow-hidden border-4 border-white shadow-md">
                    <Image
                        src={member.image || "/logo-512.png"} // Fallback to logo if no image
                        alt={member.name}
                        width={128}
                        height={128}
                        className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 ${!member.image ? 'p-2 bg-gray-50' : ''}`}
                    />
                </div>
            </div>
